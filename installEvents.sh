...
sudo docker exec -it cli1 peer chaincode install -n mycc -v 1.0 -p "/opt/gopath/src/github.com" -l "node"
sleep 3
echo 'Installing chaincode on Peer2'
sudo docker exec -it cli2 peer chaincode install -n mycc -v 1.0 -p "/opt/gopath/src/github.com" -l "node"
